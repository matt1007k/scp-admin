...
class ReporteController extends Controller
{
  ...
  public function yearReport(): View
  {
    $breadcrumbs = [
    ...
    return view('admin.reports.year', compact('breadcrumbs'));
  }

  public function yearsReport(): View
  {
    $breadcrumbs = [
      ["link" => "/dashboard", "name" => "Home"],
      ["name" => "Reporte por Rango de Años"]
    ];

    return view('admin.reports.years', compact('breadcrumbs'));
  }

  public function voucherReport(): View
  {
    $breadcrumbs = [
      ["link" => "/dashboard", "name" => "Home"],
      ["name" => "Reporte Boleta de Pago"]
    ];

    return view('admin.reports.voucher', compact('breadcrumbs'));
  }

  public function judicialsReport(): View
  {
    $breadcrumbs = [
      ["link" => "/dashboard", "name" => "Home"],
      ["name" => "Reporte Judiciales"]
    ];

    return view('admin.reports.judicials', compact('breadcrumbs'));
  }
  ...
  public function porAnios($params_code)
  {
    ...
    if ($dni) {
      ...
      if (count($years_exist_unique) > 0) {
        ...
        abort(404, 'Pago no ha sido encontrado');
      }
    } else {
      abort(404, 'La persona no existe');
    }
  }
  ...
  public function byVoucher($params_code)
  {
    // $params = base64_decode($params_code);
    $params_decode = (new EncryptService($params_code))->getDecodeEncrypt();
    $params = (new JsonParseService($params_decode))->getJsonDecode();
    ...
    if ($dni) {
      ...
      $pago = Pago::where('anio', $params->anio)->where('mes', $params->mes)
        ->where('persona_id', $params->persona_id)->first();
      ...
      if ($pago) {
        ...
        return abort(404,  'Pago no ha sido encontrado');
      }
    }
  }
  ...
  public function byJudicial($params_code)
  {
    ...
    if ($judicial) {
      ...
      return abort(404, 'Sin reportes judiciales para este periodo.');
    }
  }
}
