<form wire:submit.prevent="handleSubmit">
    <div class="grid gap-5" x-data="paymentForm" x-init="mount" x-cloak>
        @if ($message !== '')
            <div class="p-4 rounded-lg bg-green-500 text-white dark:bg-green-300 dark:text-black">{{ $message }}
            </div>
        @endif
        <div class="grid grid-cols-1 sm:grid-cols-4 md:grid-cols-5 gap-5">
            <div>
                ...
                <div class="relative">
                    ...
                    <ul class="shadow-lg rounded-sm box absolute z-50 min-w-[200px]" x-show="openSearchPerson"
                        @click.outside="openSearchPerson = false" @keydown.escape="openSearchAsset = false"
                        ...
                        @forelse($people as $person)
                            <li class="py-3 px-4 hover:bg-gray-100 dark:hover:bg-slate-800 cursor-pointer"
                                wire:click="selectedPerson({{ $person }})">
                                <h5 class="font-bold">{{ $person->full_name }}</h5>
                                <p class="text-gray-500 dark:text-gray-300">Número de Documento: {{ $person->dni }}</p>
                                <p class="text-gray-500 dark:text-gray-300">Código Modular:
                                    {{ $person->codigo_modular }}</p>
                            </li>
                        @empty
            ...
            <div class="grid gap-1">
                ...
                <select id="year" wire:model="year"
                    class="form-control border @error('year') border-red-500 @enderror">
            ...
            <div class="grid gap-1">
                ...
                <select id="month" wire:model="month"
                    class="form-control border @error('month') border-red-500 @enderror">
                ...
                <div class="relative">
                    ...
                    <ul class="shadow-lg rounded-sm box absolute z-50 min-w-[200px]" x-show="openSearchAsset"
                        ...
                        @forelse($assetsDB as $asset)
                            <li class="py-3 px-4 hover:bg-gray-100 dark:hover:bg-slate-800 cursor-pointer"
                                wire:click="selectedAsset({{ $asset }})">
                ...
                <div class="relative">
                    ...
                    <ul class="shadow-lg rounded-sm box absolute z-50 min-w-[200px]" x-show="openSearchDiscount"
                        ...
                        @forelse($discountsDB as $discount)
                            <li class="py-3 px-4 hover:bg-gray-100 dark:hover:bg-slate-800 cursor-pointer"
                                wire:click="selectedDiscount({{ $discount }})">
        ...
        <div>
            ...
            <div class="grid gap-3">
                <template x-if="person !== null">
                    <div
                        class="px-4 py-4 rounded-lg border border-gray-300 dark:border-slate-800 grid items-center grid-cols-1 md:grid-cols-3 gap-5">
                        ...
                        <div>
                            ...
                            <p class="text-gray-500 dark:text-gray-300" x-text="getModularCode(person.codigo_modular)">
                            </p>
                        </div>
                        <div>
            ...
            <div class="grid gap-3">
                <template x-if="assets.length > 0">
                    <template x-for="asset in assets">
                        ...
                        <div
                            class="px-4 py-4 rounded-lg border border-gray-300 dark:border-slate-800 grid items-center grid-cols-1 md:grid-cols-5 gap-5">
                            ...
                            <div>
                                <span class="px-2 py-1 rounded-full capitalize"
                                    ...
                                    x-text=" asset.es_imponible ? 'Asegurable' : 'No Asegurable'">
                                </span>
                            </div>
                            <div>
                            ...
                            <div class="flex justify-end">
                                <button type="button" @click="deleteAsset(asset.id)"
                                    class="text-danger dark:text-gray-100 tooltip zoom-in" title="Eliminar">
            ...
            <div class="grid gap-3">
                <template x-if="discounts.length > 0">
                    <template x-for="discount in discounts">
                        ...
                        <div
                            class="px-4 py-4 rounded-lg border border-gray-300 dark:border-slate-800 grid items-center grid-cols-1 md:grid-cols-4 gap-5">
                            <div class="col-span-1 md:col-span-2">
                            ...
                            <div>
                                <label class=" form-label font-medium">Monto S/</label>
                                <input class="form-control" x-model="discount.monto" type="number"
                                    step="0.01" />
                            </div>
                            <div class="flex justify-end">
                                <button type="button" @click="deleteDiscount(discount.id)"
                                    class="text-danger dark:text-gray-100 tooltip zoom-in" title="Eliminar">
                                    <x-heroicon-o-trash class="w-6 h-6 lucide lucide-trash" />
                                </button>
                            </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            ...
            Alpine.data('paymentForm', () => ({
                ...
                getAssetsTotal() {
                    return Number(this.assets.reduce((total, item) => total += Number(item.monto), 0))
                        .toFixed(2);
                },
                getDiscountsTotal() {
                    return Number(this.discounts.reduce((total, item) => total += Number(item.monto),
                        0)).toFixed(2);
                },
                getLiquidTotal() {
                ...
                getImponibleTotal() {
                    return Number(this.assets.filter((item) => item.es_imponible).reduce((total,
                        item) => total += Number(item.monto), 0)).toFixed(2);
                },
