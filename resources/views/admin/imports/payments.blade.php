...
@section('head', 'Importar Pagos')

@section('subcontent')

<div>
  <div class="intro-x mt-10 flex items-center h-10">
    <h1 class="text-xl font-medium truncate mr-5">Importar Pagos</h1>
  </div>

  <div class="mt-5 pb-10 w-full border-b-2 border-gray-300 dark:border-slate-700 flex flex-col justify-center items-center gap-5" x-data="ImportPayment" x-init"init">
    <div class="dropzone w-full md:w-2/4 hover:bg-gray-100 dark:hover:bg-slate-800">
      ...
      <template x-if="!uploading">
        <p class="grid place-items-center gap-2">
          ...
          <span>Nota: Es recomendable una archivo con el nombre. Ejemplo: 202202A.xlsx</span>
        </p>
      </template>
      <template x-if="uploading">
        <div>
  ...
  <div class="w-full md:w-3/5 mx-auto pb-5">
    <livewire:import-histories.payments />
  </div>
</div>
