<!-- BEGIN: Top Bar -->
<div class="h-[70px] z-[51] relative border-b border-white/[0.08] mt-12 md:-mt-5 -mx-3 sm:-mx-8 px-3 sm:px-8 md:pt-0 mb-12">
  ...
  <div class="h-full flex items-center">
      ...
      <label class="cursor-pointer flex items-center ">
        ...
        <div class="mr-3 w-8 h-8">
          <button id="theme-toggle" type="button" class="tooltip " title="Modo Oscuro/Claro">
            <x-feathericon-moon id="theme-toggle-dark-icon" class="text-primary hidden font-medium p-2 ml-3 w-8 h-8  bg-gray-100 rounded-md transition cursor-pointer hover:bg-gray-200 border" />
            <x-feathericon-sun id="theme-toggle-light-icon" class="hidden text-warning font-medium p-2 ml-3 w-8 h-8 bg-gray-700 rounded-md transition cursor-pointer" />
          </button>
        </div>
    ...
    <div class="z-10 intro-x dropdown w-8 h-8">
      <div class="dropdown-toggle w-8 h-8 rounded-full overflow-hidden shadow-lg image-fit zoom-in scale-110" role="button" aria-expanded="false" data-tw-toggle="dropdown">
        ...
        @else
        <div class="w-8 h-8 flex justify-center items-center bg-sky-400 rounded-full text-white">{{ substr(ucfirst(Auth::user()->name), 0, 1) }}</div>
        @endif
      </div>
      <div class="dropdown-menu w-56">
        <ul class="dropdown-content bg-primarymain/80 before:block before:absolute before:bg-black before:inset-0 before:rounded-md before:z-[-1] text-white">
          <li class="p-2">
          ...
          <li>
            ...
            <form action="{{ route('logout') }}" method="POST">
              ...
              <a href="{{ route('logout') }}" onclick="event.preventDefault();
                                 this.closest('form').submit();" class="dropdown-item hover:bg-white/5">
