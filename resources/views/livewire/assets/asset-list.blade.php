<div>
  <div class="intro-x mt-10 flex items-center h-10">
    <h1 class="text-xl font-medium truncate mr-5">Lista de Haberes</h1>
    @can('create', 'Models\HaberDescuento')
    <a href="{{ route('assets.create') }}" class="btn btn-primary ml-auto  w-24 shadow-md mr-2 ">
  ...
  <div class="mt-5 w-full border-b-2 border-gray-300 dark:border-slate-700 flex">
    <div class="py-3 px-5 cursor-pointer font-medium text-gray-500 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-800 border-b-4 hover:text-black dark:hover:text-white @if($filterValue === '1') text-black border-sky-500 bg-gray-200 dark:text-white dark:border-sky-500 dark:bg-slate-800 @endif" wire:click="filterBy('1')">Asegurable</div>
    <div class="py-3 px-5 cursor-pointer font-medium text-gray-500 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-800 border-b-4 hover:text-black dark:hover:text-white @if($filterValue === '0') text-black border-sky-500 bg-gray-200 dark:text-white dark:border-sky-500 dark:bg-slate-800 @endif" wire:click="filterBy('0')">No Asegurable</div>
  </div>
  ...
  <div class="grid grid-cols-12 gap-6 mt-5">
      ...
      <table class="table table-report -mt-2 text-base	">
        ...
        <tbody>
          ...
          <tr class="intro-x">
            ...
            <td>
              <span class="font-medium  uppercase">{{ $asset->nombre }}</span>
            </td>
            <td class="text-left table-cell">
              {{ $asset->descripcion }}
            </td>
            <td class="text-left table-cell">
              {{ $asset->descripcion_simple }}
            </td>
            <td width="3%" class="text-left table-cell">
              {{ $asset->createdAtFormat() }}
            </td>
            <td class="table-report__action w-10">
              <div class="flex justify-center items-center">
                @can('update', $asset)
                <a href="{{ route('assets.edit', $asset) }}" class="text-primary mr-3 tooltip dark:text-gray-100 zoom-in " title="Editar">
                  <x-heroicon-o-pencil-square class="w-6 h-6 lucide lucide-edit" />
                </a>
                @endcan
                @can('forceDelete', $asset)
                <a href="#" class="text-danger dark:text-gray-100 tooltip zoom-in" data-tw-toggle="modal" data-tw-target="#delete-modal-{{ $asset->id }}" title="Eliminar">
          ...
          @include('admin.assets.modal-delete')
          @endforeach
        </tbody>
      </table>
    </div>
    ...
    <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
      ...
      'var' => $assets,
      'route' => 'assets.create',
      'text' => 'Empiece por crear un Haber.',
      ])
    </div>
