<div>
  ...
  <div class="mt-5 w-full border-b-2 border-gray-300 dark:border-slate-700 flex">
    ...
    <div class="py-3 px-5 cursor-pointer font-medium text-gray-500 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-800 border-b-4 hover:text-black dark:hover:text-white @if($filterValue === 'sobreviviente') text-black border-sky-500 bg-gray-200 dark:text-white dark:border-sky-500 dark:bg-slate-800 @endif" wire:click="filterBy('sobreviviente')">Sobreviviente</div>
    <div class="py-3 px-5 cursor-pointer font-medium text-gray-500 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-800 border-b-4 hover:text-black dark:hover:text-white @if($filterValue === 'cesante') text-black border-sky-500 bg-gray-200 dark:text-white dark:border-sky-500 dark:bg-slate-800 @endif" wire:click="filterBy('cesante')">Cesante</div>
  </div>
          ...
          <tr>
            ...
            <th wire:click.prevent="sortBy('dni')" class="border-b-2 whitespace-nowrap">
              <a href="#" role="button" class="flex items-center rounded-md uppercase">
                @include('partials._sort-icon', ['field' => 'dni'])DNI / NÚMERO DE DOCUMENTO
              </a>
            </th>
          ...
          <tr class="intro-x">
            ...
            <td class="table-report__action w-10">
              <div class="flex justify-center items-center">
                @can('update', $person)
                <a href="{{ route('people.edit', $person) }}" class="text-primary mr-3 tooltip dark:text-gray-100 zoom-in " title="Editar">
                  <x-heroicon-o-pencil-square class="w-6 h-6 lucide lucide-edit" />
                </a>
                @endcan
                @can('forceDelete', $person)
                <a href="#" class="text-danger dark:text-gray-100 tooltip zoom-in" data-tw-toggle="modal" data-tw-target="#delete-modal-{{ $person->id }}" title="Eliminar">
